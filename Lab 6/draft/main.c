...
int main()
{
    ...
    // Dynamically allocate memory for an array of pointers for each line
    char **lines = (char **)malloc(MaxLines * sizeof(char *));
    for (int i = 0; i < MaxLines; i++)
    {
        lines[i] = (char *)malloc((MaxCharPerLine + 1) * sizeof(char));  // +1 for null terminator
        for (int j = 0; j < MaxCharPerLine + 1; j++)
        {
            lines[i][j] = '\0';  // Manually initialize each character to null
        }
    }
